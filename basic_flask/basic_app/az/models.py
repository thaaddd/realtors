...
class Broker(db.Model):
    ...
    def cache(cls):
        Broker.query.delete()
        for broker_row in get_objects(cls.event_target, cls.target_file):
            if broker_row["LicCategory"] != "Real Estate":
                continue
            broker_dict = cls.clean_data(broker_row)
            broker = Broker(**broker_dict)
